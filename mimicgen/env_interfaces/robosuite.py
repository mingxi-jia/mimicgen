...
class MG_DrawerOpening(RobosuiteInterface):
    """
    Corresponds to robosuite MugCleanup task and variants.
    """
    def get_object_poses(self):
    ...
    def get_subtask_term_signals(self):
        ...
        # eef_drawer_dist = np.linalg.norm(eef_pos - drawer_pos)
        # signals["open"] = int(
        #     (self.env.sim.data.qpos[self.env.drawer_qpos_addr] < -0.04) and (eef_drawer_dist > 0.24)
        # )

        # final subtask is placing the mug into the drawer and closing the drawer (motion relative to drawer) - but final subtask signal not needed
        return signals

class MG_DrawerClosing(RobosuiteInterface):
    """
    Corresponds to robosuite MugCleanup task and variants.
    """
    def get_object_poses(self):
    ...
    def get_subtask_term_signals(self):
        ...
        # eef_drawer_dist = np.linalg.norm(eef_pos - drawer_pos)
        # signals["close"] = int(
        #     (self.env.sim.data.qpos[self.env.drawer_qpos_addr] > 0.09) and (eef_drawer_dist > 0.24)
        # )

        # final subtask is placing the mug into the drawer and closing the drawer (motion relative to drawer) - but final subtask signal not needed
        return signals

class MG_BlockLifting(RobosuiteInterface):
    """
    Corresponds to robosuite MugCleanup task and variants.
    """
    def get_object_poses(self):
    ...
    def get_subtask_term_signals(self):
        ...
        """
        signals = dict()
        # first subtask is grasping the cube (motion relative to drawer)
