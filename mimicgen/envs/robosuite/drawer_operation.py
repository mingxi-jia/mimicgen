...
class DrawerOpening(SingleArmEnv_MG):
    """
    ...
    def _load_model(self):
        ...
        # mug only serves as a distractor here
    ...
    def _get_drawer_model(self):
        ...
        self.drawer = DrawerObject(name="DrawerObject", initial_joint_position=-0.1)
    ...
    def _check_success(self):
        ...
        drawer_closed = self.sim.data.qpos[self.drawer_qpos_addr] < -0.04
...
class DrawerOpening_D0(DrawerOpening):
    """Rename base class for convenience."""
    pass


class DrawerOpening_D1(DrawerOpening_D0):
    """
    Wider initialization for drawer on the right.
    """
    def _get_initial_placement_bounds(self):
        return dict(
            drawer=dict(
...
class DrawerOpening_D2(DrawerOpening_D0):
    """
    Wider initialization for drawer on the left.
    """
    def _get_initial_placement_bounds(self):
        return dict(
            drawer=dict(
                x=(-0.15, 0.05),
                y=(-0.25, -0.35),
                z_rot=(-np.pi / 6. + np.pi, np.pi / 6. + np.pi),
                reference=self.table_offset,
            ),
            object=dict(
                x=(-0.25, 0.15),
...
class DrawerClosing_D0(DrawerOpening):
    def _check_success(self):
        """
        Check if task is complete.
        """
        ...
        # check for closed drawer
        drawer_closed = self.sim.data.qpos[self.drawer_qpos_addr] > 0.09

        return drawer_closed

class DrawerClosing_D1(DrawerClosing_D0):
    """
    Wider initialization for drawer on the right.
    """
    def _get_initial_placement_bounds(self):
...
class DrawerClosing_D2(DrawerClosing_D0):
    """
    Wider initialization for drawer on the left.
    """
    def _get_initial_placement_bounds(self):
